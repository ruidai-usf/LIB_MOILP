 ...
 * Author: Rui Dai
 *
 * Created on February 25, 2017, 8:17 AM
 */
...
ILOSTLBEGIN
IloEnv env;
IloModel PSM_IP_model(env); /* This Is Your Model Name */
IloCplex PSM_IP_cplex(PSM_IP_model); /* This is Your Solver Name */
...
void Create_The_CPLEX_Model() {
    ...
    for (int i = 0; i < Var_num; i++) {
        ...
        var_x[i] = IloNumVar(env, 0.0, Infinity, ILOBOOL, VarName);
    }
...
public:
    ...
    Box() {
        ...
        point_solution = new double [Var_num];
    ...
    void Generate_New_Nondominated_Points() {
        ...
        Extra_Constraints.add(Math_Expression - vertex_B[0] <= -epsilon);

        PSM_IP_model.add(Extra_Constraints);
        ...
        Extra_Constraints.add(Math_Expression - vertex_T[1] <= -epsilon);

        PSM_IP_model.add(Extra_Constraints);

        PSM_IP_cplex.extract(PSM_IP_model);

        //        PSM_IP_cplex.exportModel("Model.lp");

        PSM_IP_cplex.setOut(env.getNullStream());

        PSM_IP_cplex.setParam(IloCplex::EpGap, Optimality_Gap);

        PSM_IP_cplex.resetTime();
        if (PSM_IP_cplex.solve() == 0) {
            cout << "Infeasible" << endl;

            Num_of_IPs++;
            IP_Runtime += PSM_IP_cplex.getTime();
        } else {
            Num_of_IPs++;
            IP_Runtime += PSM_IP_cplex.getTime();
            is_ND_point = 1;
            ...
            for (int i = 0; i < Var_num; i++) {
                point_solution[i] = PSM_IP_cplex.getValue(var_x[i]);
            }
        ...
        PSM_IP_model.remove(Extra_Constraints);
...
void Create_The_Box_ZERO() {
    ...
    PSM_IP_model.add(Objective_Func);

    /*generate present cplex model*/
    PSM_IP_cplex.extract(PSM_IP_model);

    //    PSM_IP_cplex.exportModel("Model1.lp");
    ...
    Initial_Box->vertex_B[1] = PSM_IP_cplex.getObjValue();

    //    cout << Initial_Box->vertex_B[1] << endl;

    PSM_IP_cplex.clear();
    PSM_IP_model.remove(Objective_Func);
    ...
    //    PSM_IP_cplex.exportModel("Model2.lp");
    ...
    Initial_Box->vertex_B[0] = PSM_IP_cplex.getObjValue();
    ...
    //    PSM_IP_cplex.exportModel("Model3.lp");
    ...
    Initial_Box->vertex_T[0] = PSM_IP_cplex.getObjValue();
    ...
    if (Is_one_ND_point == 0) {
        ...
        //        PSM_IP_cplex.exportModel("Model4.lp");
        ...
        Initial_Box->vertex_T[1] = PSM_IP_cplex.getObjValue();
...
int main(int argc, char** argv) {
    ...
    for (int j = 0; j < Var_num; j++) {
        Math_Expression += (Obj1_coef[j] + Obj2_coef[j]) * var_x[j];
    }
